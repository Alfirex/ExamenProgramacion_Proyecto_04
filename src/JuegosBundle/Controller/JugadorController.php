...
class JugadorController extends Controller
{
    ...
    //-- Examen --\\
    //-- MODIFICACIÓN 01 --\\
    public function buscarJugadorAction($nombre)
    {
        $repository = $this->getDoctrine()->getRepository('JuegosBundle:Jugador');
        $mostrar = $repository->findByNombre($nombre);
        return $this->render('JuegosBundle:Carpeta_Jugador:mostrarPorNombreJugadores.html.twig',array('TablaJugadores' => $mostrar ));
    }
    //-- MODIFICACIÓN 02 --\\
    public function buscarJugadoridAction($id)
    {
        $repository = $this->getDoctrine()->getRepository('JuegosBundle:Jugador');
        $mostrar = $repository->find($id);
        return $this->render('JuegosBundle:Carpeta_Jugador:mostrarPorNombreJugadoresId.html.twig',array('TablaJugadores' => $mostrar ));
    }
